...
def write_to_csv(file, venue, team1, team2, MR, MRN, MW, OR, ORN, OW, win):
    ...
    with open('seven_fifteen_overs.csv', 'a') as testfile:     # append it data to the csv file
        ...
        for innings in innings_list:
            ...
            try:
                for ball in match['innings'][innings_list.index(innings)][innings]["deliveries"]:
                    for key in ball:
                        if float(key) >= 6.0 and float(key) < 15:
                            ball_count += 1
                            runs += ball[key]["runs"]["total"]
                            if "wicket" in ball[key]:
                                wicket += 1
                if ball_count > 54:     # to handle the case when less than six overs are bowled
                    balls = 54
                else:
                    balls = ball_count
