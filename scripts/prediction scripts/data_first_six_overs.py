...
def write_to_csv(file, venue, team1, team2, MR, MRN, MW, OR, ORN, OW, win):
    ...
    with open('first_six_overs.csv', 'a') as testfile:     # append it data to the csv file
                    ...
                    for key in ball:
                        if float(key) < 6:
                            ...
                            if "wicket" in ball[key]:
                                wicket += 1
                if ball_count > 36:     # to handle the case when less than six overs are bowled
                    balls = 36
                else:
                    balls = ball_count
